...
import {DATABASE, HOST} from '../__mocks__/connectionDetails.js';
import {asDate, asTime, asTimestamp, query} from '../dist/index.js';
...
describe('Database interactions', () => {
    ...
    describe('Queries', () => {
        ...
        it('Check query escaping', async () => {
            const records = await testLayout.records.list({ portals: {}, limit: 10 })
                ...
                .fetch();
            equal(records.length, 0, 'Query escaping/sanitization failed. Generated query that failed: ' + JSON.stringify(query `=${'*'}`));
        });
        it('Check query escaping using iterable search', async () => {
            ...
            equal(foundCount, 0, 'Query escaping/sanitization failed. Generated query that failed: ' + JSON.stringify(query `=${'*'}`));
        });
    ...
    describe('Containers', () => {
        it('Test uploading', async () => {
            const file = new File([new TextEncoder()
