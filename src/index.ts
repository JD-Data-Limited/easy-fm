...
export class Layout {
     ...
     */
    createRecord(): Promise<RecordBase | Error | void> {
     ...
     */
    getRecord(recordId): Promise<RecordBase> {
...
class LayourRecordManager {
    ...
    create(): Promise<RecordBase | Error | void> {
        return new Promise((resolve, reject) => {
            ...
            this.layout.getLayoutMeta().then(layout => {
                ...
                resolve(new LayoutRecord(this.layout, -1, 0, fields))
            }).catch(e => {
                reject(e)
            })
        })
    ...
    get(recordId): Promise<RecordBase> {
        return new Promise((resolve, reject) => {
            let record
...
export class Portal {
    ...
    constructor(record: RecordBase, name: string) {
        this.record = record
        this.name = name
    }
}
    ...
    private generateQueryParams(extraBody: extraBodyOptions = {}) {
        ...
        if (extraBody.scripts) {
            if (extraBody.scripts.prerequest) {
                ...
                if (extraBody.scripts.prerequest.param) params.push("script.prerequest.param=" + extraBody.scripts.prerequest.param)
            }
            if (extraBody.scripts.presort) {
                params.push("script.presort=" + extraBody.scripts.presort.name)
                if (extraBody.scripts.presort.param) params.push("script.presort.param=" + extraBody.scripts.presort.param)
            }
            if (extraBody.scripts.after) {
                params.push("script=" + extraBody.scripts.after.name)
                if (extraBody.scripts.after.param) params.push("script.param=" + extraBody.scripts.after.param)
            }
        }
